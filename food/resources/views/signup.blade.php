...
<div class="flex items-center justify-between bg-gray-950 text-white py-3 px-4 fixed top-0 z-0 w-full">
    ...
    <div class="flex items-center space-x-6">
       ...
       @auth
        <a href="" class="text-white hover:text-blue-800">{{auth()->user()->name}}</a>
        <a href="{{route('logout')}}" class="text-white hover:text-blue-800">Logout</a>
        @endauth

        @guest
        <a href="{{route('signup')}}" class="text-white hover:text-blue-800">Create an account</a>
...
<div class="bg-white p-8 rounded shadow-md w-96 mt-20">
    <h2 class="text-2xl font-semibold mb-6 text-black">Register Here</h2>
    <form action="" method="post">
        @csrf
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-gray-600">Name</label>
            <input type="name" id="name" name="name" value="{{ old('name') }}" class="mt-1 p-2 w-full border rounded-md">
            @error('name')
    ...
    <a href="{{ route('login') }}" class="text-black">Already registered</a>
</div>
<script>
